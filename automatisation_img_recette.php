...
if (isset($_POST["valider"])) {
    // je prepare l'id et le le nom de la photo de la recette pour comparaison
    $req = $pdo->prepare("Select id,picture from soup");
    $req->execute();
    $soupes = $req->fetchAll(PDO::FETCH_ASSOC);
    // je boucle dans les images recueper et je prend le nom pour comparaison
    foreach ($_FILES["images"]["name"] as $coucou) {
        //je boucle dans les soupes pour avoir le nom de la soupe et l'id pour comparaison
        foreach ($soupes as $soupe) {
            //je teste pour savoir si il y a un match entre la photo et un des fichier uploader
            if ($coucou == $soupe["picture"]) {
                // je stocke l'id de la soupe 
                $id = $soupe["id"];
                //je teste pour savoir quel est l'index de la photo voulu
                for ($i = 0; $i < count($_FILES["images"]["name"]); $i++) {
                    if ($_FILES["images"]["name"][$i] == $soupe["picture"]) {
                        ...
                        $req_id = $pdo_img->prepare("Select id from img_test");
                        ...
                        if ($exist) {
                            ...
                            $req_finale = $pdo_img->prepare("INSERT INTO img_test(id, nom, taille, type, bin)VALUES(?, ?, ?, ?, ?)");
                            // j'envois les information de la photo upbloader dans la base de donnée avec l'identifiant de 
                            $req_finale->execute($tableau);
                            var_dump($req_finale);
                            if (isset($req_finale)) {
                                echo "l'image : $id, à bien été envoyer dans la base de donnée";
                            }
                        }
                    }
                }
